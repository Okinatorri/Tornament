...
import { addTournamentGame, getTournamentGamesByName } from "../../api";

export const useHistoryState = (tournamentName) => {
    ...
    const [tourGames, setTourGames] = useState([]);
    ...
    useEffect(() => {
        const fetchTournamentGames = async () => {
            ...
            try {
                setLoading(true);
                const result = await getTournamentGamesByName(tournamentName);
                if (result.success) {
                    if (result.games.length === 0) {
                        setIsNoActive(true);
                    } else {
                        setTourGames(result.games);
    ...
    return { rounds, loading, error, isNoActive, tourGames };
};
